...
function SuggestDateForm({ onSubmit, onCancel }) {
  ...
  const [duration, setDuration] = useState(86400000); // 1 Día por defecto
...
export default function Voting() {
  ...
  const [activeTab, setActiveTab] = useState("fechas"); // 'fechas' | 'lugares'
  ...
  // 🛡️ LECTURA DEL DEADLINE GLOBAL (Con el Caballo de Troya activado)
  ...
  // ── 🌟 EL MOTOR DE RELOJ INMORTAL ──────────────────────────────────────
  // En lugar de depender de React que se "marea", este motor lee directamente de Zustand
  // cada segundo, de esta forma jamás captura datos viejos ni se borra al ir al Home.
  useEffect(() => {
    const intervalId = setInterval(() => {
      // Pedimos la data más fresca directo a la bóveda (Zustand)
      ...
      if (vState.date?.isActive && vState.date?.deadline) {
        ...
        if (remaining > 0) {
          ...
          // ¡TIEMPO AGOTADO! Cierre automático de fechas
      ...
      if (vState.location?.isActive && vState.location?.deadline) {
        ...
        if (remaining > 0) {
          ...
          // ¡TIEMPO AGOTADO! Cierre automático de lugares
  ...
  }, []); // Cero dependencias = Motor siempre encendido sin reinicios

  // ── FUNCIONES DE AGREGADO CON "CABALLO DE TROYA" ───────────────────────
