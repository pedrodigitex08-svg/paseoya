...
// ─────────────────────────────────────────────
// INGREDIENT CARD (Item de la lista - CON COMPARATIVA RESTAURADA)
// ─────────────────────────────────────────────
function IngredientCard({
  ...
  // 🔥 LÓGICA DE COMPARATIVA MATEMÁTICA
  const hasActualCost =
...
// ─────────────────────────────────────────────
// ADD INGREDIENT FORM (Modal Express Reducido)
// ─────────────────────────────────────────────
...
function BusConfigCard({ bus, participants, onUpdate, onSave, isLocked }) {
  ...
  const perPerson =
    bus.assignedParticipants.length > 0 && bus.totalCost > 0
      ? Math.ceil(bus.totalCost / bus.assignedParticipants.length)
      : 0;
...
export default function Logistics() {
  ...
  // 🔥 ESTADOS PARA ACORDEONES Y PLATOS INTELIGENTES
  const [expandedCategory, setExpandedCategory] = useState(null);
  const [platoInputs, setPlatoInputs] = useState({});
  const [platoLocked, setPlatoLocked] = useState({}); // Memoria para saber si se ocultó la caja
  ...
  // 🔥 Función para bloquear/ocultar la caja
  ...
  // 🔥 Función para abrir la caja (lapicito)
    ...
    <div className="min-h-screen bg-slate-50 pb-24 relative">
      ...
      <div className="bg-white border-b border-slate-100 px-4 pt-12 pb-0 sticky top-0 z-40">
        <div className="flex items-center gap-3 mb-4">
          ...
          {/* 🔥 EL NUEVO SELLO DE MARCA SUTIL (ISOTIPO) 🔥 */}
          <div
      ...
      <div className="px-4 pt-4 space-y-4">
        {/* ───────────────────────────────────────────────────────── */}
        {/* 🥘 TAB: MENÚ (SISTEMA DE ACORDEONES Y PLATOS OCULTOS)     */}
        {/* ───────────────────────────────────────────────────────── */}
        {activeTab === "menu" && (
          <div className="animate-in fade-in duration-300">
            {/* 🔥 BANNER DE MARKETING FOMO SIEMPRE VISIBLE 🔥 */}
                  ...
                  >
                    ...
                    {isExpanded && (
                      <div className="p-4 border-t border-slate-100 animate-in slide-in-from-top-2 duration-200">
                        {/* Renderizado de items agrupados por plato */}
                        {itemsInCat.length > 0 && (
                        ...
                        {!isLocked && (
                          <div className="bg-slate-50 rounded-xl p-3 border border-slate-100 space-y-3">
                            ...
                            platoInputs[cat.id]?.trim() ? (
                              ...
                              /* ── ESTADO 1: ESCRIBIENDO (CAJA ABIERTA) ── */
