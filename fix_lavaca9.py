...
if 'const getLabels =' not in content:
    content = content.replace('export default function LaVaca() {', getLabelsCode + '\nexport default function LaVaca() {')

# Find the block inside ParticipantPayRow
content = re.sub(
...
# And if there's a second one (e.g. at line 1250)
# Let's just do it globally:
content = re.sub(
    r'  const isShortEvent =\s*paseo\.category === "rumba".*?lblPdfMercado = "Extras / Propinas:";\n  }',
    '  const { lblAlojamiento, lblMercado, lblMercadoMini, lblPdfHospedaje, lblPdfMercado, isShortEvent } = getLabels(paseo);',
    content,
...
with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)
print("Updated LaVaca.jsx cleanly")
